...
Docker image can no longer be run by root user. Docker users must run ```sudo chown user:user ~/.peerswap -R```, where user is your user:group that you use to run docker and .peerswap is the folder where peerswap data is located. Also note the new folder mapping:
